...
export const PhoneField = ({label, required, fieldEvent, disabled, placeholder, helperText, zIndex, testId, fullWidth}: PhoneFieldProps) => {
        ...
        if (phoneNumber === '') {
            ...
            if (required) {
                if (phoneNumberWithoutSpace.length === 8) {
                    fieldEvent({ value: phoneNumberWithoutSpace, status: 'valid', event: 'blur' });
                    setValid(true);
                } else {
                    fieldEvent({ value: phoneNumberWithoutSpace, status: 'inValid', event: 'blur' });
                    setValid(false);
                }
            } else {
                fieldEvent({ value: phoneNumberWithoutSpace, status: 'optional', event: 'blur' });
                setValid(true);
            }
        }
    }
